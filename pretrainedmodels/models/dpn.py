...
class InputBlock(nn.Module):
    def __init__(self, num_init_features, kernel_size=7,
        ...
        self.conv = nn.Conv1d(
            2, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
        self.bn = nn.BatchNorm1d(num_init_features, eps=0.001)
        self.act = activation_fn
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
        ...
        self.last_linear = nn.Conv1d(in_chs, num_classes, kernel_size=1, bias=True)

    def logits(self, features):
        if not self.training and self.test_time_pool:
            x = F.avg_pool1d(features, kernel_size=7, stride=1)
            out = self.last_linear(x)
            # The extra test time pool should be pooling an img_size//32 - 6 size patch
            out = adaptive_avgmax_pool1d(out, pool_type='avgmax')
        else:
            x = adaptive_avgmax_pool1d(features, pool_type='avg')
            out = self.last_linear(x)
...
def adaptive_avgmax_pool1d(x, pool_type='avg', padding=0, count_include_pad=False):
    """Selectable global pooling function with dynamic input kernel size
    """
    if pool_type == 'avgmaxc':
        x = torch.cat([
            F.avg_pool1d(
                x, kernel_size=(x.size(2), ), padding=padding, count_include_pad=count_include_pad),
            F.max_pool1d(x, kernel_size=(x.size(2), ), padding=padding)
        ], dim=1)
    elif pool_type == 'avgmax':
        x_avg = F.avg_pool1d(
                x, kernel_size=(x.size(2), ), padding=padding, count_include_pad=count_include_pad)
        x_max = F.max_pool1d(x, kernel_size=(x.size(2), ), padding=padding)
        x = 0.5 * (x_avg + x_max)
    elif pool_type == 'max':
        x = F.max_pool1d(x, kernel_size=(x.size(2), ), padding=padding)
    else:
        if pool_type != 'avg':
...
class AdaptiveAvgMaxPool1d(torch.nn.Module):
    ...
    def __init__(self, output_size=1, pool_type='avg'):
        ...
        if pool_type == 'avgmaxc' or pool_type == 'avgmax':
            self.pool = nn.ModuleList([nn.AdaptiveAvgPool1d(output_size), nn.AdaptiveMaxPool1d(output_size)])
        elif pool_type == 'max':
